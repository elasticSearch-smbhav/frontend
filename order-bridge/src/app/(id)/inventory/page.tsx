...
const TableComponent = ({
  ...
  const fetchOrders = async () => {
    try {
      ...
      setStatusFilterData(response.data.listings);
    } catch (error) {
            ...
            <tbody>
              {statusFilterData.map((order, index) => (
                ...
                >
                  ...
                  <td className="px-6 py-4">{order["Purchases"]}</td>
                  <td className="px-6 py-4">{order["Mrp"]}</td>
        ...
        <div className="flex items-center justify-start gap-6">
          <div className="flex items-center justify-end gap-2">
            ...
            <Dropdown label={status} size="xs" color={"secondary"}>
              ...
              <Dropdown.Item onClick={() => setStatus("BUYABLE")}>
                Buyable
              </Dropdown.Item>
              <Dropdown.Item onClick={() => setStatus("DISCOVERABLE")}>
                Discoverable
              </Dropdown.Item>
              <Dropdown.Item onClick={() => setStatus("DELETED")}>
                Deleted
              </Dropdown.Item>
            </Dropdown>
          </div>
